 ...
 */
@SuppressLint("ViewConstructor") class DayView extends LinearLayout {
  ...
  // View
  private AppCompatCheckedTextView mTvDay;
  ...
  public DayView(Context context, CalendarDay day) {
    ...
    View v = li.inflate(R.layout.day_view, DayView.this, false);
    addView(v);
    mTvDay = findViewById(R.id.day);
  ...
  public void setDayFormatter(DayFormatter formatter) {
    ...
    CharSequence currentLabel = mTvDay.getText();
    Object[] spans = null;
  ...
  private void setEnabled() {
    ...
    if (!isInMonth && shouldBeVisible) {
      mTvDay.setTextColor(mTvDay.getTextColors().getColorForState(
  ...
  private void regenerateBackground() {
    if (selectionDrawable != null) {
      mTvDay.setBackgroundDrawable(selectionDrawable);
    } else {
      mCircleDrawable = generateBackground(selectionColor, fadeTime, circleDrawableRect);
      mTvDay.setBackgroundDrawable(mCircleDrawable);
    }
  }
  ...
  void applyFacade(DayViewFacade facade) {
    ...
    // Set image
    if(facade.getImageLayout() == "garment") {
      assert facade.getNumImages() == 1;

      mImageLayout.setRowCount(1);
      mImageLayout.setColumnCount(1);

      ImageView image = new ImageView(mContext);
      mImageLayout.addView(image);
      Glide.with(mContext)
              .load(facade.getImage(0))
              .apply(new RequestOptions().override(150, 150))
              .into(image);

    } else if(facade.getImageLayout() == "outfit") {
      assert facade.getNumImages() == 4;
      mImageLayout.setRowCount(2);
      mImageLayout.setColumnCount(2);

      for(int i = 0; i < 4; ++i) {
        ImageView image = new ImageView(mContext);
        mImageLayout.addView(image);
        Glide.with(mContext)
                .load(facade.getImage(i))
                .apply(new RequestOptions().override(75, 75))
                .into(image);
      }
    } else if(facade.getImageLayout() == "outfit_dress") {
      assert facade.getNumImages() == 3;
      mImageLayout.setRowCount(2);
      mImageLayout.setColumnCount(2);
      ...
      for(int i = 0; i < 3; ++i) {
        ...
        if(i == 1) {
          ...
          params.rowSpec = GridLayout.spec(0, 2); // First cell in first column use columnSpan 2.
        ...
        Glide.with(mContext)
                .load(facade.getImage(i))
                .apply(new RequestOptions().override(75, height))
                .into(image);
      }
    }
  }
  ...
  private void calculateBounds(int width, int height) {
    ...
    if (width >= height) {
      ...
      tempRect.set(0, height-width, width, height);
      circleDrawableRect.set(0, 0, width, width);
    }
  }

  public void setChecked(boolean checked) {
      mTvDay.setChecked(checked);
  }

  public boolean isChecked() {
    return mTvDay.isChecked();
  }

  public void setTextAppearance(Context context, int resId) {
    mTvDay.setTextAppearance(context, resId);
  }
}
